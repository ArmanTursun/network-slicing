...
from bqr_scenario_creator import create_bqr_agent
...
class Evaluator():
    ...
    def evaluate(self, i):
        ...
        bqr_agent = create_bqr_agent(rng, self.scenario, all_scenarios, quantile = self.quantile, 
                                     embb_sla = embb_sla, mmtc_sla = mmtc_sla, qr_params = QR_PARAMS, slots_per_step = SLOT_PER_STEP )
        print('run {}: QR agent created'.format(i))
        results = bqr_agent.run(node_env, TRAIN_STEPS, EGENT_TRAIN_BATCH_SIZE)
