import numpy as np
import math
import matplotlib.pyplot as plt
import os
import sys
from mpl_toolkits.axes_grid1.inset_locator import inset_axes, mark_inset

def average_per_window(values, window):
    values = np.array(values)
    n = len(values)
    trimmed_length = (n // window) * window  # Drop extra elements
    reshaped = values[:trimmed_length].reshape(-1, window)
    return reshaped.mean(axis=1)

def movingaverage(values, window):
    weights = np.repeat(1.0, window)/window
    sma = np.convolve(values, weights, 'valid')
    return sma

WINDOW = 100
separate = True
# Define your source and target folders
source_folder = './results/scenario_0/QR_15_0_5_2_01_cost5'
# Generate distinct colors from a colormap
color_list = plt.cm.tab10.colors  # up to 10 distinct colors; you can also use tab20, Set3, etc.
color_map = {i: color_list[i % len(color_list)] for i in range(10)}

fig, axs = plt.subplots(nrows=6, ncols=5, figsize=(17, 17), constrained_layout=False)
axs = axs.flatten()
#fig.subplots_adjust(top=0.78)

labels = []

# Loop over the two files
if separate:
    for i in range(30):
        labels.append('result_' + str(i))
        filename = f'results_{i}.npz'
        if filename in os.listdir(source_folder):
            source_path = os.path.join(source_folder, filename)
        else:
            continue

        # Load both source and target
        source_data = np.load(source_path, allow_pickle=True)

        # Get 'reset' array from source
        _violations_total = source_data['violation']
        _violations_total = np.array([np.sum(r) for r in _violations_total], dtype=np.int16)
        violations_total = movingaverage(_violations_total.cumsum(), WINDOW)

        viol_raw = source_data['violation']
        _violations = np.array([np.array(r, dtype=np.int16) for r in viol_raw])  # object array
        _violations = np.stack(_violations, axis=0)  # now shape (T, K)
        _violations = _violations.T
        violations = [movingaverage(v, WINDOW) for v in _violations]

        violations_idx = i

        steps = np.arange(len(violations_total))

        axs[violations_idx].set_title(labels[i], fontsize=14)
        #axs[violations_idx].plot(steps, violations_total, label = f'{labels[i]}_total', linewidth = 2, color=color_map[9])
        for j, v in enumerate(violations):
            axs[violations_idx].plot(np.arange(len(v)), v, label = f'{labels[i]}_slice{j}', linewidth=2)
        axs[violations_idx].set_xlabel('Epoch', fontsize=14)  # Add an x-label to the axes.
        axs[violations_idx].set_ylabel('violations', fontsize=14)
        axs[violations_idx].set_ylim((0,0.2)) # 15000
        axs[violations_idx].set_yticks(np.arange(0, 0.21, 0.02))
        axs[violations_idx].legend(loc='upper left', fontsize=14)
        #axs[1].legend(loc='best')
        axs[violations_idx].grid()
        #if i == 9:
        #    ncol = math.ceil(5)
        #    fig.legend(labels, loc='upper center', ncol=ncol, bbox_to_anchor=(0.5, 1.0), frameon=True, fontsize=14)
        #    fig.tight_layout(rect=[0, 0, 1, 0.8])
        fig.tight_layout()
        fig.savefig(source_folder + '/results_separate', format='png')
#else:
    # Placeholder: one list per slice
    per_slice_data = None

    # Load all files
    for i in range(30):
        filename = f'results_{i}.npz'
        if filename in os.listdir(source_folder):
            source_path = os.path.join(source_folder, filename)
        else:
            continue
        source_data = np.load(source_path, allow_pickle=True)

        viol_raw = source_data['violation']
        _violations = np.array([np.array(r, dtype=np.float64) for r in viol_raw])
        _violations = np.stack(_violations, axis=0).T  # shape: (K, T)

        # Apply moving average per slice
        violations = [movingaverage(v, WINDOW) for v in _violations]

        if per_slice_data is None:
            num_slices = len(violations)
            per_slice_data = [[] for _ in range(num_slices)]

        # Store each slice's series
        for j in range(num_slices):
            per_slice_data[j].append(violations[j])  # each entry becomes list of 10 arrays

    # Plot all slices
    fig, ax = plt.subplots(figsize=(12, 6))

    for j in range(num_slices):
        # shape: (10, T)
        runs = np.array(per_slice_data[j])
        min_len = min(r.shape[0] for r in runs)
        runs = np.array([r[:min_len] for r in runs])  # truncate if not aligned

        mean = np.mean(runs, axis=0)
        std = np.std(runs, axis=0)
        x = np.arange(len(mean))

        ax.plot(x, mean, label=f'Slice {j}', color=color_map[j])
        ax.fill_between(x, mean - std, mean + std, color=color_map[j], alpha=0.3)
    ax.axhline(y=0.01, color='black', linestyle='--', linewidth=2, label='99% SLA')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Violation Rate')
    ax.set_ylim((0, 0.1))
    ax.set_yticks(np.arange(0, 0.11, 0.01))
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(source_folder + '/results_all', format='png')


    # Placeholder: one list per slice
    per_slice_data = None

    # Load all files
    for i in range(30):
        filename = f'results_{i}.npz'
        if filename in os.listdir(source_folder):
            source_path = os.path.join(source_folder, filename)
        else:
            continue
        source_data = np.load(source_path, allow_pickle=True)

        viol_raw = source_data['violation']
        _violations = np.array([np.array(r, dtype=np.float64) for r in viol_raw])
        _violations = np.stack(_violations, axis=0).T  # shape: (K, T)

        # Apply moving average per slice
        violations = [movingaverage(v.cumsum(), WINDOW) for v in _violations]
        #violations = [v.cumsum() for v in _violations]

        if per_slice_data is None:
            num_slices = len(violations)
            per_slice_data = [[] for _ in range(num_slices)]

        # Store each slice's series
        for j in range(num_slices):
            per_slice_data[j].append(violations[j])  # each entry becomes list of 10 arrays

    # Plot all slices
    fig, ax = plt.subplots(figsize=(12, 6))

    for j in range(num_slices):
        # shape: (10, T)
        runs = np.array(per_slice_data[j])
        min_len = min(r.shape[0] for r in runs)
        runs = np.array([r[:min_len] for r in runs])  # truncate if not aligned

        mean = np.mean(runs, axis=0)
        std = np.std(runs, axis=0)
        x = np.arange(len(mean))

        ax.plot(x, mean, label=f'Slice {j}', color=color_map[j])
        ax.fill_between(x, mean - std, mean + std, color=color_map[j], alpha=0.3)

    ax.set_xlabel('Epoch')
    ax.set_ylabel('Violation Rate')
    ax.set_ylim((0, 100))
    ax.set_yticks(np.arange(0, 101, 10))
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(source_folder + '/results_total', format='png')


