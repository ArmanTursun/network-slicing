...
class PPO_mini:
    def __init__(self, env, lr_actor = 0.001, lr_critic = 0.001, n_steps=8, gamma = 0.99, K_epochs = 10, eps_clip = 0.2, 
                 has_continuous_action_space = False, action_std_init=0.6, verbose = 0):

        self.has_continuous_action_space = has_continuous_action_space

        self.action_std = action_std_init        # action distribution (Multivariate Normal)
        self.action_std_init = action_std_init   # starting std for action distribution (Multivariate Normal)
    ...
    def update(self):
        ...
        rewards = torch.tensor(np.array(rewards).flatten(), dtype=torch.float32).to(device)
    ...
    def learn(self, total_timesteps = 10000):
        ...
        while time_step <= total_timesteps:
            ...
            action = np.array([action])  # wraps the action in a batch
            ...
            # update PPO agent
            if time_step % self.n_steps == 0:
                self.update()
                print(f"Step: {time_step}, Cummulative Reward = {cum_reward}, Total violation = {violation_count}")
                cum_reward = 0
                violation_count = 0
